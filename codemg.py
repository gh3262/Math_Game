...
DISPLAY_ROTATION = 0
BUTTON_FONT = bitmap_font.load_font("/fonts/ComicSansMS-19.pcf")
SMALL_BUTTON_FONT = bitmap_font.load_font("/fonts/ComicSansMS-15.pcf")
TITLE_FONT = bitmap_font.load_font("/fonts/EffectsEighty-32.pcf")
R_FONT = bitmap_font.load_font("/fonts/Calibri-29.pcf")
Q_FONT = bitmap_font.load_font("/fonts/Calibri-38.pcf")
SCORE_FONT = bitmap_font.load_font("/fonts/Calibri-17.pcf")
TOP_LIST_FONT = bitmap_font.load_font("/fonts/ComicSansMS-14.pcf")
...
DEBUG_FLOW = True
DEBUG_SD = True
RTC_TYPE = "ds3231"  # Hardware uses DS3231 on STEMMA I2C.
SD_MOUNT_BAUDRATES = (4000000, 2000000, 1000000, 400000)
TEST_SD_ONLY = False  # Set to True to test SD mount in isolation before display init
...
COLOR_BACKGROUND = 0x777777
COLOR_BUTTON_INNER = 0x555577
COLOR_TITLE_TEXT = 0x00FF88
COLOR_TEXT_PRIMARY = 0xFFFFFF
COLOR_RESULT_TEXT = 0xFFFF00
COLOR_ANSWER_BUTTON_1 = 0xAAAAFF
COLOR_ANSWER_BUTTON_2 = 0x44FF44
COLOR_ANSWER_BUTTON_3 = 0xFFFF44
COLOR_ANSWER_BUTTON_4 = 0xFF8888
COLOR_QUIT_BUTTON = 0xFF8888
COLOR_NEXT_BUTTON = 0x0088FF
COLOR_BUTTON_PRESSED = COLOR_TEXT_PRIMARY
COLOR_NAME_ENTRY_NAV_FLASH = 0xFFFF44
COLOR_NAME_ENTRY_BACKSPACE_FLASH = 0xFF8888
COLOR_NAME_ENTRY_ENTER_FLASH = 0x44FF44
COLOR_NAME_ENTRY_FIXED_ROW = 0xFFBBBB
COLOR_NAME_ENTRY_CHANGING_ROW = 0xAAAAFF
COLOR_NAME_ENTRY_HEADER_TEXT = COLOR_TEXT_PRIMARY
COLOR_NAME_ENTRY_VALUE_TEXT = COLOR_RESULT_TEXT
COLOR_NAME_ENTRY_THEME_HEADER_TEXT = 0x88FFFF
COLOR_NAME_ENTRY_THEME_VALUE_TEXT = 0x00FF88
...
BUTTON_COUNT = 4
BUTTON_START_X = 5
BUTTON_BOTTOM_MARGIN = 35
BUTTON_WIDTH = (SCREEN_WIDTH // BUTTON_COUNT) - 2
BUTTON_TOP = SCREEN_HEIGHT - BUTTON_HEIGHT - BUTTON_BOTTOM_MARGIN
ANSWER_BUTTON_Y_OFFSET = 10
ANSWER_BUTTON_TOP = BUTTON_TOP + ANSWER_BUTTON_Y_OFFSET
BUFFER_TOP = BUTTON_TOP + BUTTON_HEIGHT
NAME_ENTRY_TEXT_HEIGHT = 50
NAME_ENTRY_BUTTON_WIDTH = 75
NAME_ENTRY_BUTTON_HEIGHT = 45
NAME_ENTRY_GRID_X = (SCREEN_WIDTH - (NAME_ENTRY_BUTTON_WIDTH * 4)) // 2
NAME_ENTRY_GRID_Y = NAME_ENTRY_TEXT_HEIGHT + 5


def sd_log(message, detail=None):
	if not DEBUG_SD:
		return
	if detail is None:
		print("[SD] {}".format(message))
	else:
		print("[SD] {}: {}".format(message, detail))
...
def debug_sd_required_paths():
	paths_to_check = (
		SCORE_FILE_PATH,
		PLAYER_FILE_PATH,
	) + PLAYER_TEMPLATE_FILE_PATHS
	for path in paths_to_check:
		exists, reason = sd_path_exists(path)
		if exists:
			sd_log("Found", path)
		else:
			sd_log("Missing", "{} ({})".format(path, reason))
...
def create_sdcard_with_baudrate(baudrate):
	try:
		...
		return sdcardio.SDCard(spi, sd_cs), "baudrate unsupported"


def diagnose_spi_and_cs():
	"""Check SPI bus and CS pin state before SD mount."""
	sd_log("SPI diagnosis", "Checking bus state...")
	try:
		# Check if we can read SPI frequency (may not be exposed)
		sd_log("SPI object", repr(spi))
		# Try to toggle CS pin to verify it responds
		try:
			cs_test = digitalio.DigitalInOut(sd_cs)
			cs_test.direction = digitalio.Direction.OUTPUT
			cs_test.value = True
			time.sleep(0.01)
			cs_test.value = False
			time.sleep(0.01)
			cs_test.value = True
			sd_log("CS pin test", "D5 toggled successfully")
			cs_test.deinit()
		except Exception as e:
			sd_log("CS pin test failed", repr(e))
	
		# Check for card-detect pin on featherwing 3315 (typically board.D7)
		try:
			card_detect = digitalio.DigitalInOut(board.D7)
			card_detect.direction = digitalio.Direction.INPUT
			cd_state = "inserted" if not card_detect.value else "not inserted"
			sd_log("Card detect (D7)", cd_state)
			card_detect.deinit()
		except Exception as e:
			sd_log("Card detect check failed", repr(e))
	except Exception as e:
		sd_log("SPI diagnosis failed", repr(e))
...
def mount_sd_with_retries():
	...
	for baudrate in SD_MOUNT_BAUDRATES:
		try:
			sd_log("Mount attempt", "cs={} baud={}".format(sd_cs, baudrate))
			sdcard, note = create_sdcard_with_baudrate(baudrate)
			if note:
				sd_log("SDCard note", note)
			vfs = storage.VfsFat(sdcard)
			storage.mount(vfs, "/sd")
			return True, ""
		except Exception as exc:
			last_error = repr(exc)
			sd_log("Attempt failed", "baud={} err={}".format(baudrate, last_error))
...
def build_problem_set(operator, count):
	pairs = []
	if operator == "-":
...
def touch_to_pixel(raw_x, raw_y):
	...
	# XPT2046 axis orientation is swapped/inverted relative to display.
...
try:
	if "/sd" in os.listdir("/"):
		sd_log("Already mounted", "/sd")
	else:
		mounted, mount_error = mount_sd_with_retries()
		if not mounted:
			raise OSError("SD mount failed after retries: {}".format(mount_error))
	sd_log("Mounted", "/sd")
	sd_log("Root entries", os.listdir("/"))
	sd_log("/sd entries", os.listdir("/sd"))
	debug_sd_required_paths()
except Exception as exc:
	sd_log("Mount failed", repr(exc))
	debug_sd_required_paths()

if TEST_SD_ONLY:
	sd_log("TEST_SD_ONLY mode active", "Skipping display initialization to isolate SD issue")
	import sys
	sys.exit(0)
...
game_page_layout.add_content(game_page_group, page_name="game")
...
debug_i2c_scan(i2c)
ts = xpt2046_circuitpython.Touch(
	spi,
	cs=digitalio.DigitalInOut(t_cs),
	width=240,
	height=320,
	x_min=166,
	x_max=1960,
...
px = neopixel.NeoPixel(board.NEOPIXEL, 1, brightness=0.1, auto_write=True, pixel_order=neopixel.RGB)

LED_COLOR_OFF = (0, 0, 0)
LED_COLOR_BLUE = (0, 0, 255)
LED_COLOR_GREEN = (0, 255, 0)
LED_COLOR_RED = (255, 0, 0)
LED_COLOR_YELLOW = (255, 128, 0)
LED_COLOR_MAGENTA = (255, 0, 255)
LED_FEEDBACK_OFF_SECONDS = 0.25
BUZZER_STARTUP_FREQUENCY = 440
BUZZER_STARTUP_DURATION = 0.5
BUZZER_DUTY_CYCLE = 32768
BUZZER_CORRECT_FREQUENCY = 840
BUZZER_CORRECT_DURATION = 0.20
BUZZER_WRONG_FREQUENCY = 200
BUZZER_WRONG_DURATION = 1.0
BUZZER_SKIP_FREQUENCY = 500
BUZZER_SKIP_BEEP_DURATION = 0.1
BUZZER_SKIP_GAP_DURATION = 0.06
...
if False:  # change to True if you want to set the time!
	...
	t = time.struct_time((2026, 4, 24, 0, 38, 00, 5, -1, -1))
...
addition = build_problem_set("+", 144)
multiplication = build_problem_set("*", 144)
subtraction = build_problem_set("-", 78)
...
game_page_layout.show_page("game")
...
def update_clock_label(force=False):
	...
	if force or (now - last_clock_update) >= 5.0:
		...
		game_time_label.text = formatted_time
...
def update_elapsed_display(total_seconds):
	...
	total_time_label.text = "time {:02d}:{:02d}".format(minutes, seconds)
...
def update_score_displays(elapsed_seconds=None):
	...
	nbr_q_label.text = "{} / {} / {}".format(total_correct, total_attempts, total_skipped)
...
def invalidate_score_cache(game_type=None):
	global score_entries_cache, score_entries_cache_loaded, score_math_cache
...
def load_player_names(limit=PLAYER_BUTTON_LIMIT):
	...
	for _ in range(PLAYER_LOAD_RETRY_COUNT):
		...
		for player_path in PLAYER_FILE_PATHS:
			try:
				with open(player_path, "r") as players_file:
	...
	for player_path in PLAYER_FILE_PATHS:
		try:
			...
			try:
				...
				separator = ""
			with open(player_path, "a") as players_file:
...
def normalized_time_score(avg_time_value, min_time, max_time):
	# Lower average time should produce a higher normalized score.
...
def player_totals_lines(limit=TOP_SCORE_PAGE_LIMIT):
	...
	for entry in entries:
		...
		player_totals[player_name]["problems"] += entry["nbr_q"]
...
def show_post_score_summary(player_name, current_entry, saved, candidate_composite_score, high_score_status=""):
	...
	if wrong_answers < 0:
		wrong_answers = 0
	score_title_label.text = "Game Result"
	score_question_label.anchor_point = (0.0, 0.0)
	score_question_label.anchored_position = (5, 68)
	score_question_label.color = COLOR_TEXT_PRIMARY
	score_question_label.text = "{} Score for {}:".format(player_name, score_type_label(current_entry["type"]))
...
def show_post_score_comparison_page():
	...
	score_title_label.text = "Your Ranking"
	score_question_label.anchor_point = (0.0, 0.0)
	score_question_label.anchored_position = (5, 48)
	score_question_label.text = "{} across all types".format(post_score_player_name)
	score_question_label.color = COLOR_TEXT_PRIMARY
...
def show_post_score_top_scores_page():
	...
	score_title_label.text = "Top Scores"
	score_question_label.anchor_point = (0.0, 0.0)
	score_question_label.anchored_position = (5, 48)
	score_question_label.text = "{} Top {}".format(type_label, TOP_SCORE_PAGE_LIMIT)
	score_question_label.color = COLOR_TEXT_PRIMARY
...
def show_start_screen_top_scores_page(score_type_index=0):
	...
	recent_page_index = len(SCORE_TYPES) + 1
	...
	if score_type_index < len(SCORE_TYPES):
		...
		if score_type_index < len(SCORE_TYPES) - 1:
			...
			set_score_next_text("Next: {}".format(next_label))
		else:
			set_score_next_text("Next: Totals")
	elif score_type_index == totals_page_index:
		score_title_label.text = "Player Totals"
		score_question_label.text = "Games/Problems Played"
		top_lines = player_totals_lines(TOP_SCORE_PAGE_LIMIT)
		set_score_next_text("Next: Last 2 Days")
	else:
		score_title_label.text = "Last 2 Days"
		score_question_label.text = "Combined Games/Problems"
		top_lines = player_recent_totals_lines(TOP_SCORE_PAGE_LIMIT)
		set_score_next_text("Next: Home")
...
def show_name_choices_for_score(status_message="Tap NEW if not listed"):
	global game_started, mode_select_active, selecting_problem_count, selecting_score_name, post_score_status_active, post_score_page_index, selectable_player_names, player_name_page_index, start_screen_top_scores_active
	show_game_page()
	game_started = False
	...
	question_label.text = "Choose Player Name"
...
def finish_completed_game():
	...
	high_score_status = ""
	if score_math["candidate"] is not None:
...
def choose_player_name(choice_text):
	...
	if choice_text not in selectable_player_names:
		return
	current_player_name = choice_text
	selecting_score_name = False
	show_problem_type_choices()
...
def show_current_problem():
	...
	question_label.text = problem_text(problem)
...
def show_problem_type_choices():
	global game_started, mode_select_active, selecting_problem_count, selecting_score_name, post_score_status_active, post_score_page_index, name_entry_message, start_screen_top_scores_active
	show_game_page()
	game_started = False
	mode_select_active = True
	selecting_problem_count = False
	...
	set_button_text(quit_button, "Start")
	set_button_text(next_button, "Next")
	question_label.text = "Problem Type?"
	result_label.text = ""
...
def show_problem_count_choices():
	...
	set_button_text(quit_button, "Start")
	set_button_text(next_button, "Next")
	question_label.text = "Nbr Problems?"
	result_label.text = ""
...
def show_title_screen(reset_title=True):
	...
	result_label.text = ""
	score_title_label.text = "Score"
	score_question_label.text = ""
...
def choose_problem_type(choice_text):
	global active_questions, current_operator_symbol, current_game_type
	if choice_text in ("Add", "Addition"):
		active_questions = addition
		current_operator_symbol = "+"
		current_game_type = "Addition"
		title_label.text = "Addition"
	elif choice_text in ("Subtract", "Subtraction"):
		active_questions = subtraction
		current_operator_symbol = "-"
		current_game_type = "Subtraction"
		title_label.text = "Subtraction"
	elif choice_text in ("Multiply", "Multiplication"):
		active_questions = multiplication
		current_operator_symbol = "*"
		current_game_type = "Multiplication"
		title_label.text = "Multiply"
	elif choice_text == "Mixed":
		active_questions = []
		current_operator_symbol = "+"
		current_game_type = "Mixed"
		title_label.text = "Mixed"
	else:
		return
...
def choose_problem_count(choice_text):
	global problem_count_target
	if choice_text in ("10", "20", "35", "50"):
		problem_count_target = int(choice_text)
		start_game()
...
while True:
	...
	touch = None
	try:
		touch = ts.get_coordinates()
	except Exception:
		touch = None

	if touch_release_required and touch is not None:
		pending_pressed = None
		pending_press_count = 0
		no_touch_count = 0
		...
		no_touch_count = 0
		#print(gc.mem_free())
		tx, ty = touch_to_pixel(touch[0], touch[1])
		pressed = button_from_touch(tx, ty)

		if pressed is None:
			pending_pressed = None
			pending_press_count = 0
		elif pressed is pending_pressed:
			pending_press_count += 1
		else:
			pending_pressed = pressed
			pending_press_count = 1

		if pressed is not None and pressed is not last_pressed and pending_press_count >= 2:
			...
			print("{} pressed at ({}, {})".format(pressed["name"], tx, ty))

			if pressed["role"] == "quit":
				if selecting_score_name:
					result_label.text = "Choose player to continue"
				elif start_screen_top_scores_active:
					show_name_choices_for_score()
					screen_changed = True
				elif game_started:
					show_title_screen()
				...
				if selecting_score_name:
					...
					advance_start_screen_top_scores_page()
					screen_changed = True
				elif game_started:
			...
			elif pressed["role"] == "name_entry":
				choice_text = pressed["label"].text
				if choice_text:
					starting_page = current_page_name
					handle_name_entry_choice(choice_text)
					screen_changed = current_page_name != starting_page

			elif pressed["role"] == "answer" and mode_select_active:
				choice_text = pressed["label"].text
				if choice_text:
					if selecting_score_name:
						choose_player_name(choice_text)
					elif selecting_problem_count:
						choose_problem_count(choice_text)
					else:
						choose_problem_type(choice_text)
					screen_changed = True

			elif pressed["role"] == "answer" and game_started:
		...
		if touch_release_required:
			touch_release_required = False
		pending_pressed = None
		pending_press_count = 0
		no_touch_count += 1
		if last_pressed is not None and no_touch_count >= 2:
			set_button_color(last_pressed, last_pressed["base_color"])
			last_pressed = None
