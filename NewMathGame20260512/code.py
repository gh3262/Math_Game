...
def touch_to_pixel(raw_x, raw_y):
    """Convert raw touch controller coordinates to display pixel coordinates."""
    # Touch controller axes are rotated relative to display axes.
    x = int((raw_y - TOUCH_Y_MIN) * (display_width - 1) / (TOUCH_Y_MAX - TOUCH_Y_MIN))
    y = int((raw_x - TOUCH_X_MIN) * (display_height - 1) / (TOUCH_X_MAX - TOUCH_X_MIN))
...
debug_i2c_scan(i2c)
irq_dio = None
ts = adafruit_tsc2007.TSC2007(i2c, irq=irq_dio, invert_x=True)

px = neopixel.NeoPixel(board.A0, 1, brightness=0.1, auto_write=True, pixel_order=neopixel.RGB)
...
if True:  # change to True if you want to set the time!
	...
	t = time.struct_time((2026, 5, 13, 9, 36, 00, 3, -1, -1))
...
def show_current_problem():
	...
	set_label_text_if_changed(question_label, problem_text(problem))
